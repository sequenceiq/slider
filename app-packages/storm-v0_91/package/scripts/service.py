...
def service(
  ...
  pid_chk_cmd = format("{jps_path} -vl | grep \"backtype.storm.daemon.{name}\" {grep_and_awk}  > {pid_file}")
  ...
  if action == "start":
    if name == "rest_api":
      ...
      cmd = format("env JAVA_HOME={java64_home} PATH=$PATH:{java64_home}/bin {storm_bin} {name}")
